...
function BlockTable(){
    ...
    return (
        ...
        <div className="mt-8 h-[19rem] overflow-y-auto border border-gray-300 rounded">
  <div className="max-w-full overflow-x-auto">
    <table className="w-full border-collapse rounded-lg">
      <thead className="bg-gray-800 text-white sticky top-0">
        <tr>
          <th className="border-b-2 border-gray-600 px-4 py-2 rounded-tl-lg">Blockchain</th>
          <th className="border-b-2 border-gray-600 px-4 py-2">Chain ID</th>
          <th className="border-b-2 border-gray-600 px-4 py-2">Button</th>
          <th className="border-b-2 border-gray-600 px-4 py-2 rounded-tr-lg">Status</th>
        </tr>
      </thead>
          ...
          <tr key={item.id}>
            <td className="border-gray-600 px-4 py-2">
              <div className="flex items-center">
                ...
                <img className="h-4 w-4" src={item.imgS} alt="Blockchain Icon" />
              </div>
            </td>
            <td className="border-gray-600 px-4 py-2 text-center">{item.chainselector}</td>
            <td className="border-gray-600 px-4 py-2 text-center">
              <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-1 px-3 rounded text-sm " onClick={handleDeploy}>Deploy</button>
            </td>
            <td className="border-gray-600 px-4 py-2 text-center rounded-br-lg">
              {statuss === 'Active' ? (
                <span className="text-green-500 animate-pulse">Active</span>
              ) : (
                <span className="text-red-500 animate-pulse">Not Active</span>
              )}
            </td>
          </tr>
        ))}
